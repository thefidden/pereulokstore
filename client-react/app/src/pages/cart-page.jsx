...
import { registerOrder } from "../utils.js";
                ...
                ? <div className = 'cart'>
                    <div className = 'productsGrid'>{
                        ...
                        user.cart.map((cartItem, index) =>
                            <CartItem cartItem = {cartItem} user = {user} setUser = {setUser}
                                      animationDelay = {0.3 + 0.1 * index}/>
                        )
                    }</div>

                    <div className = 'totalPrice'>Сумма: {
                        user && user.cart.reduce((accumulator, cartItem) => accumulator + cartItem.product.price * cartItem.amount, 0)
                    }₽
                    </div>

                    <div className = 'buyButtonWrapper'>
                        <div className = 'buyButton' onClick = {registerOrder}>Оплатить</div>
                    </div>
                </div>
