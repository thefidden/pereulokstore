"""Конфигурация Django Admin для моделей API."""
...
from api.models.ProductReport import ProductReport
from api.models.ProductTag import ProductTag
from api.models.Tag import Tag


class ProductImageInline(admin.TabularInline):
    """Inline-класс для управления изображениями товара в админке."""
    model = ProductImage
    extra = 1
    fields = ('image',)
    readonly_fields = ()
...
class ProductTagInline(admin.TabularInline):
    """Inline-класс для управления тегами товара в админке."""
    model = ProductTag
    extra = 1
...
class ProductAdmin(ImportExportModelAdmin):
    """Админка для модели Product с поддержкой экспорта и генерации отчетов."""
    list_display = ('name', 'type', 'price', 'creation_date', 'link', 'remaining')
    list_display_links = ('name',)
    list_filter = ('type', 'price', 'creation_date', 'remaining')
    ...
    def link(self, obj: Product):
        """Отображает кликабельную ссылку на товар.
        
        Args:
            obj: Экземпляр модели Product.
            
        Returns:
            SafeString: HTML-ссылка на товар.
        """
        return format_html(
    ...
    def generate_pdf_report(self, request, queryset):
        """Генерирует PDF-отчет по выбранным товарам.
